...
pacman::p_load(pacman, fs, tidyverse, skimr, janitor, kableExtra, dplyr, purrr, rio, here, tidyverse, dplyr, ggplot2, ggpubr, stringr, DT, shiny, plotly, ggExtra, cowplot, patchwork, forcats, naniar, DescTools, lubridate)

```

```{r eval=FALSE, include=FALSE}
### Import data file
mortdf <- import(here("Datasets", "Homeless deaths by month.csv"))

mortdf$Year <- paste(mortdf$`Month of death`,
                     mortdf$`Year of death`, sep = " ")

# Convert Year column values to "class" *date*
mortdf2 <- mortdf %>% mutate(Year = lubridate::my(Year))

### Change Column Names using **rename** function of "dplyr" package
lookup <- c(Date = "Year", Year = "Year of death")
mortdf2 <- rename(mortdf2, all_of(lookup))

# select columns for new df
mortdf3 <- select(mortdf2, 1,2,5,4) %>% drop_na()

mortdf4 <- mortdf3 %>% group_by(Year) %>% summarise(Year_sum = sum(Count)) %>% kable()

mortdf3 %>% ggplot() +
  aes(x = Year, weight = Count) +
  geom_bar(fill = "darkblue") +
  labs(x = "Date", y = "# Deaths (counts)") +
  theme_bw() +
  theme(axis.title.y = element_text(size = 12L, face = "bold"),
        axis.title.x = element_text(size = 12L, face = "bold"))

### Use "fs" package to generate file "tree" of folder "Datasets"
datasetFiles <- fs::dir_tree(path="Datasets")
...
### (Current Data: 2020/11/03 - 2024/02/29)
...
### (Current Data: 2020/10/30 - 2024/02/29)
...
```{r include=FALSE}
### Import data file
DSOSOdt <- import(here("Datasets", "DSOSO_2021to10Apr2024.csv"))
...
**Figure 5. Total Shelter Population within the Greater Toronto Area (GTA).** The above plot shows the total number of daily shelter users for the period spanning 2021-01-01 to 2024-04-10. The number of shelter occupants have increased by more than 60% during this time.
...
**Figure 6. Shelter Population per GTA region.** The plot shows the total number of shelter users within different parts of the **GTA** (i.e. Toronto, North York, Etobicoke, Scarborough, Vaughan) for the past 39 months (2021-01-01 to 2024-04-10). The data reveals that the vast majority of shelter users are currently being housed within Toronto and North York (see Bar graph in **Figure 7**).
...
### Use "group by" and "sum" functions to sum up observations
### Total Population per "CITY"
df240329 <- subset(df3.NA, OCCUPANCY_DATE == "2024-04-10")
CityTots <- df240329 %>%
  group_by(OCCUPANCY_DATE, CITY) %>%
  summarise(n_observ = n(), sum_Users = sum(SERVICE_USER_COUNT))
...
**Figure 7. Current Shelter Populations per GTA Region.** This bar graph shows the current (2024-04-10) number of shelter occupants within each of the designated **GTA** regions. Toronto (67.5%) and North York (18.9%) shelter the majority of homeless people, while Scarborough (8.9%), Etobicoke (4.1%), and Vaughan (0.6%) collectively house less than 14% of the homeless population.  
...
**Figure 10. Homeless Overnight Service Types within the GTA.** Homeless individuals within the **GTA** can access different types of overnight services, inclduing: **(i) Shelters**, **(ii) 24-Hour Respites**, **(iii) Motels/Hotels**, **(iv) Interim Housing**, **(v) Warming Centres**, **(vi) 24-Hour Women’s Drop-ins**, and **(vii) Isolation/Recovery Sites**. There are a few other **Service Types** offered, but they are ill-defined and accessed by very few people. As of March 29<sup>th</sup> 2024 the vast majority of **Service Types** are emergency **Shelters** (4815 of 9587, or 50.2%) and **Motel/Hotel** (4093 of 9587, or 42.7%) services. Unfortunately, programs that ultimately lead to <u>permanent</u> housing are virtually non-existent.
...
<div align=center>
## Figure 12: Homes First - Mixed Adult Delta Hotel Shelter
### (DSOSO: # Occupants)
...
##################################################
##################################################
### DATA FOR ==> ***LOESS MODEL***
##################################################
##################################################
LOESS1 <- df3NAPID16011 %>% filter(OCCUPANCY_DATE >= "2024-01-04" & OCCUPANCY_DATE <= "2024-04-7")

LOESS2 <- LOESS1 %>% select(OCCUPANCY_DATE, Room_Capacity_Funding, Rooms_Occupied, Rooms_Unavailable)

LOESSdf <- na.omit(LOESS2)

### Convert all character variables to factor variables
library(dplyr)
LOESSdf %>% mutate_if(is.character, as.factor)

### Fit a ***LOESS MODEL***
loess_model <- loess(Rooms_Unavailable ~ as.numeric(OCCUPANCY_DATE), data = LOESSdf, span = 0.5, degree = 1)

# Fit higher order polynomial LOESS model
loess_model2 <- loess(Rooms_Unavailable ~ as.numeric(OCCUPANCY_DATE), data = LOESSdf, span = 0.5, degree = 2)

# Generate predictions from the LOESS model
predictions <- predict(loess_model, newdata = LOESSdf)
predictions <- cbind(LOESSdf, predictions)
predictions <- predictions[order(predictions$Rooms_Unavailable), ]

predictions2 <- predict(loess_model2, newdata = LOESSdf)
predictions2 <- cbind(LOESSdf, predictions2)
predictions2 <- predictions2[order(predictions2$Rooms_Unavailable), ]

# Create a scatter plot of the original data
plot(predictions$OCCUPANCY_DATE,
     predictions$Rooms_Unavailable,
     col = "darkblue",
     main = "LOESS Regression",
     xlab = "Date",
     ylab = "Unavaiable Rooms") +
lines(predictions$OCCUPANCY_DATE,
      predictions$predictions,
      col = "darkred", lwd = 1)

LOESSplot1 <- ggplot(LOESSdf, aes(x = OCCUPANCY_DATE, y = Rooms_Unavailable)) +
  geom_point() +
  stat_smooth(method = "loess", se = FALSE, span = 0.5,
              method.args = list(degree = 1) )

LOESSplot2 <- ggplot(LOESSdf, aes(x = OCCUPANCY_DATE, y = Rooms_Unavailable)) +
  geom_point() +
  stat_smooth(method = "loess", se = FALSE, span = 0.5,
              method.args = list(degree = 2) )

LOESSdf$residuals <- residuals(loess_model)
LOESSdf$residuals2 <- residuals(loess_model2)

LOESSplot3 <- ggplot(LOESSdf, aes(x = OCCUPANCY_DATE, y = residuals)) +
  geom_point() +
  stat_smooth(method = "loess", se = FALSE, span = 0.5,
              method.args = list(degree = 1) )

LOESSplot4 <- ggplot(LOESSdf, aes(sample = residuals)) + 
  stat_qq(distribution = qnorm) +
  stat_qq_line(distribution = qnorm, col = "blue")

LOESSplot5 <- ggplot(LOESSdf, aes(sample = residuals2)) + 
  stat_qq(distribution = qnorm) +
  stat_qq_line(distribution = qnorm, col = "blue")
...
<div align=center>
## Figure 13: Homes First - Mixed Adult Delta Hotel Shelter
...
plot16011 <- df3NAPID16011 %>%
  filter(OCCUPANCY_DATE >= "2024-01-04" & OCCUPANCY_DATE <= "2024-04-10") %>%
  ggplot() +
...
**Figure 12 & 13. Homes First's Delta Hotel Shelter.** The two graphs above show the number of rooms at the Delta Hotel Shelter that are either occupied (**Figure 12**) or unoccupied (**Figure 13**) by a homeless person. The marginal histogram in **Figure 12** shows that the plotted occupancy values fall within 2 distinct populations. These two populations of data points begin to emerge on July 22<sup>nd</sup> 2022 (i.e. July 21<sup>st</sup> = 316 residents, July 22<sup>nd</sup> = 334 residents). The reasons for this are currently unknown, but may reflect <u>increased</u> funding to the Shelter System. Although the number of occupants have been steadily decreasing between Feb 5<sup>th</sup> to April 4<sup>th</sup>, the recent influx of refugees has rapidly corrected these falling numbers. These rapid changes in the number of people sheltered at the Delta Hotel over the past 2 months is also reflected in the number of unavailable rooms at the shelter (**Figure 13**). The fitted **Loess** (**LO**cal regr**ESS**ion) curve is fairly <u>well defined</u> (i.e. residuals are fairly constant over the chosen range of x values, with the exception of the last three data points which have relatively high negative residual values). Nevertheless, it is important to note that the overnight **Delta Hotel Shelter** (Program ID 16011) supervised by **Homes First** is, according to **DSOSO** records, a <u>designated</u> mixed adult (Sector) shelter that falls under the emergency COVID-19 Program. So it is by definition <u>not</u> a refugee-only shelter space. According to **Homes First**, which runs many other additional shelter and supportive housing sites, their mission is to "*develop and provide affordable, stable housing and support services to break the cycle of homelessness for people with the fewest housing options*". However, despite the continuing rise in the number of homeless persons in Toronto (**Fig. 1, 2**), and the recent influx of Federal dollars (**<a class="one" href="https://www.cbc.ca/news/canada/toronto/asylum-seeker-shelter-funding-toronto-1.7102849">$162 million</a>**) to support the Toronto shelter system, **Homes First** has unilaterally decided to transition the **Delta** Hotel Shelter to a refugee only site (i.e. no consultations with residents). Apparently, in accordance with the wishes of local Ward 22 councillor Nick Mantas, the shelter is actively trying to move or evict people from the safety of the shelter. Many have already faced punitive "*service restrictions*" without any appeals process. Recent updates to the system has generated a new "*faux*" appeals process that typically results in the clients eviction at the end of the restriction period. It is important to note that during the time it takes to resolve the appeal process the client is removed from the shelter and forced to find accommodations on their own (i.e. homeless). Homes First's blatant exclusion of Delta residents from any decision making process that directly affects their immediate and future well being is imprudent and overtly punitive in nature. Many residents are afraid that they will be forced to move back onto the streets, or to some unsafe open congregate shelter space. The lack of transparency about decisions that directly affect the lives of Delta residents has only heightened resident's fears, anxiety and distrust of Homes First. The current drive to restrict shelter services to refugees at the expense of non-refugees is perplexing given the recent influx of Federal funding into the Toronto Shelter system. Available resources, if used wisely, could safely house homeless people within the **GTA** without resorting to an outdated, expensive and dehumanizing emergency shelter system (**Note:** number of **UNAVAILABLE ROOMS** = **ROOMS CAPACITY FUNDING** - **ROOM OCCUPIED**, **Table 3**). 
...
**7.** City of Toronto - Housing Stability Service System Overview (2019). "*Housing + Homelessness Service Glossary*". https://www.toronto.ca/city-government/data-research-maps/research-reports/housing-and-homelessness-research-and-reports/housing-stability-service-system-map-and-terms/
