---
title: "GTA Homeless Shelters & Resources"
author: "V4UL"
date: "`r Sys.Date()`"
output: 
...
pacman::p_load(
  flexdashboard,
  flextable, ### table cell formatting properties
...
HR Map {data-icon="ion-stats-bars"}
===================================== 

Column {data-width=500}
-----------------------------------------------------------------------

### <b>GTA Homless Resources (HR) Map</b>
...
### <b>Table: GTA Homeless Resources (HR)</b>
...
ft1 <- flextable(ToHRes.data1) %>% autofit()
ft1 <- set_table_properties(x = ft1,
                            opts_html = list(
...
HS Map {data-icon="ion-stats-bars"}
===================================== 

Column {data-width=475}
-----------------------------------------------------------------------

### <b>GTA Homeless Shelters (HS) Map (2024-05-03)</b>

```{r include=FALSE}
#### read in data file
TS.data <- readr::read_csv(here("Datasets", "DSOSO_4May2024.csv"))
TS2.data <- select(TS.data, 4:19)
...
#### Centre the Map view to Yonge & Bloor, and set the zoom level
TSmap <- TSmap %>% setView(-79.38681892548695,43.670216153303066,
                   zoom = 12)

#### Add markers with custom labels for each Resource site
TSmap2 <- TSmap %>% addMarkers(data = TS2.data,
                               label = paste("Name: ",
                                             TS2.data$LOCATION_NAME,
                                             "<br>",
                                             "Location: ",
                                             TS2.data$ADDRESS,
                                             "<br>",
                                             ...
                                             "<br>",
                                             "Program Model: ",
                                             TS2.data$PROGRAM_MODEL,
                                             "<br>",
                                             "Service Type: ",
                                             TS2.data$SERVICE_TYPE,
                                             "<br>",
                                             "Program Area: ",
                                             TS2.data$PROGRAM_AREA,
                                             "<br>",
                                             "# Occupants: ",
                                             TS2.data$SU_count) %>%
                                 lapply(htmltools::HTML))
...
### <b>Table: GTA Homeless Shelters (HS) (2024-05-03)</b>

```{r}
# select columns for new df to create table
TS2.data1 <- select(TS2.data, 3,4,11:15)

### Change Column Names using **rename** function of "dplyr" package
NewOldCol <- c(Name = "LOCATION_NAME", Address = "ADDRESS", Sector = "SECTOR", Model = "PROGRAM_MODEL",  Service = "SERVICE_TYPE", Area = "PROGRAM_AREA", Occupants = "SU_count")
TS2.data1 <- rename(TS2.data1, all_of(NewOldCol))

set_flextable_defaults(font.size = 11, font.color = "black",
                       border.color = "black", padding = 2,
                       table.layout = "autofit")

ft2 <- flextable(TS2.data1) %>% autofit()
ft2 <- set_table_properties(x = ft2,
                            opts_html = list(
                              scroll = list(freeze_first_column = TRUE)))
...
CanPT.plot <- ggplot(CanPT.pit) +
  aes(x = Date, fill = Stat, weight = VALUE,
      text = paste('Date: ', as.Date(Date), '<br>Stat: ', Stat, '<br>Type: ', Type, '<br>Count: ', VALUE)) +
  ...
  facet_wrap(vars(GEO), scales = "free_y", ncol = 3L, nrow = 5L
  )

### Create Interactive Plot with "ggplotly" package
CanPT.plotly <- ggplotly(CanPT.plot, tooltip = "text")
...
PitPlot <- ggplot(Pit.df2.ca) +
  aes(x = Date, fill = Statistics, weight = VALUE,
      text = paste('Date: ', as.Date(Date), '<br>Stat: ', Statistics, '<br>Count: ', VALUE)) +
  geom_bar(position = "dodge") +
  ...
  theme(axis.title.y = element_text(size = 12L, face = "bold"),
        ...
        panel.grid.minor = element_blank(),
        legend.position = "none") +
  facet_wrap(vars(Stype), scales = "free_y", ncol = 3L, nrow = 1L)

### Create Interactive Plot with "ggplotly" package
PitPlotly <- ggplotly(PitPlot, tooltip= "text")
...
### <b>GTA Shelter System - Population in Crisis</b>
...
<br>
**(ii)** **<a class="one" href="https://open.toronto.ca/dataset/toronto-shelter-system-flow/">Toronto Shelter System Flow</a>** (**TSSF**)**<sup>8</sup>** data is compiled on a monthly basis by the city of Toronto's **S**helter **M**anagement **I**nformation **S**ystem (**SMIS**). These records show the number of people entering (intake) or leaving (outtake) the shelter system. Some of the terms used in these records are listed in **Table 2**. Unfortunately, the current **SMIS** data is limited since it does not include any information about homeless people sleeping outdoors, or homeless people who do not use city funded services that are apart of **SMIS**. Although it is difficult to assess the exact number of homelessness people within the city, the most recent **<a class="one" href="https://open.toronto.ca/dataset/2021-street-needs-assessment-results/">Street Needs Assessment (2021)</a>** suggests that ~18% of city's homeless population are not included in the current data. This is likely an under estimate of its "*true*" size given the severity of the current housing and homelessness problems within the region.**<sup>8</sup>**
<br>
**(iii) <a class="one" href="https://open.toronto.ca/dataset/daily-shelter-overnight-service-occupancy-capacity/">Daily Shelter Overnight Service Occupancy</a>** (**DSOSO**)**<sup>9</sup>** records maintained by **SMIS** is compiled on a daily basis. It records the number of people who use overnight shelters and allied services within the **GTA**. The data includes the identify of program operators, as well as the locations and nature of the services they provide (**Table 3**).**<sup>6</sup>**<br>
&nbsp; &nbsp; Unfortunately there is little information about the amount of funding the city receives and allocates for these different homeless programs and services. The **DSOSO** data only mentions **Funding Capacity** within the context of the "*number of beds or rooms that a program is intended* [funded] *to provide*".**<sup>9</sup>** Although there may be a number of reasons why beds or rooms may be temporarily out of service (e.g. repairs, renovations, outbreaks, or pest control) they are not reported. These records also mention **Actual Capacity**, which refers to the "*number of beds or rooms in service and showing as <u>available for occupancy</u> in the SMIS at time of reporting.*" This latter measure is touted as being "*a more effective capacity measure to assess program occupancy rates*". However, the number of <u>unavailable</u> rooms or beds (**Note:** **# UNAVAILABLE BEDS** = **Capacity Funding Bed** - **Capacity Actual Bed**) also tells us the number of rooms/beds the current system is funding but not using on a daily basis. Unfortunately there appears to be little oversight of the service providers with regards to reducing or eliminating bed/room "*unavailability*", and the draconian nature of the service restrictions they impose on clients. 

<br>

<div class="DivCenter90">
<b>TABLE 1: CENTRAL INTAKE CALL CENTRE FEATURES</b><br>
```{r echo=FALSE, error=FALSE}
#### read in data file
Tab.data <- readr::read_csv(here("Datasets", "CIC_TSSF_DSOSO_Table_Terms.csv"))

### select columns for "CIC" Table
CIC.Tab <- slice(Tab.data, 1:4)

set_flextable_defaults(font.size = 12, font.color = "black",
                       border.color = "black",
                       padding = 2, table.layout = "autofit")

CICft <- flextable(CIC.Tab) %>%
  bold(bold = T, part = "header") %>%
  bold(bold = T, i = 1:4, j = 1, part = "body") %>%
  align(align = "center", part = "header") %>%
  bg(part = "header", bg = "gray60") %>%
  bg(i = c(1,3), part = "body", bg = "gray90") %>% 
  autofit()
...
<b>TABLE 2: TSSF DATA FEATURES</b><br>
```{r echo=FALSE, error=FALSE}
### select columns for "TSSF" Table
TSSF.Tab <- slice(Tab.data, 5:15)

# Ensure VALID UTF-8 encoded characters in the `Name` column
Encoding(x = TSSF.Tab$Term) <- "UTF-8"
Encoding(x = TSSF.Tab$Definition) <- "UTF-8"

# replace all non UTF-8 character strings with an empty space
TSSF.Tab$Definition <- iconv(x = TSSF.Tab$Definition,
                          from = "UTF-8",
                          to = "UTF-8",
                          sub = "")
...
TSSFft <- flextable(TSSF.Tab) %>%
  bold(bold = T, part = "header") %>%
  bold(bold = T, i = 1:11, j = 1, part = "body") %>%
  align(align = "center", part = "header") %>%
  bg(part = "header", bg = "gray60") %>%
  bg(i = c(1,3,5,7,9,11), part = "body", bg = "gray90") %>% 
  autofit()
...
<b>TABLE 3: DSOSO DATA FEATURES</b><br>
```{r echo=FALSE, error=FALSE}
### select columns for "DSOSO" Table
DSOSO.Tab <- slice(Tab.data, 16:26)

# Ensure VALID UTF-8 encoded characters in the `Name` column
Encoding(x = DSOSO.Tab$Term) <- "UTF-8"
Encoding(x = DSOSO.Tab$Definition) <- "UTF-8"

# replace all non UTF-8 character strings with an empty space
DSOSO.Tab$Definition <- iconv(x = DSOSO.Tab$Definition,
                          from = "UTF-8",
                          to = "UTF-8",
                          sub = "")
...
<div align=center>
**Figure 8: Toronto Shelter System Flow (TSSF)**<br>
**(SMIS dataset: 2018/01 - 2024/03)**<br>

```{r}

### Apply **rowSums** function to add up "returned" and "new" users
Demodf4$RNusers <- rowSums(Demodf4[ , c(4:6)], na.rm=TRUE)
Demodf5 <- select(Demodf4, 1:9,20,15,16)

Demodf5.plot <- ggplot(Demodf5) +
  aes(x = Date, y = RNusers,
      colour = moved_to_housing,
      group = population_group) +
  geom_line(linewidth = 1) +
  scale_color_viridis_c(option = "inferno", direction = 1) +
  guides(colour = guide_colourbar(direction = "horizontal")) +
  labs(x = "Date",
       y = "# New & Returned Shelter Occupants",
       colour = " ") +
  theme_bw() +
  theme(axis.title.y = element_text(size = 10L, face = "bold"),
        axis.title.x = element_text(size = 10L, face = "bold"),
        plot.background = element_rect(fill = "white"),
        panel.grid.minor = element_blank(),
        legend.justification = c(1,0),
        legend.position = c(0.575,0.08)) +
  facet_wrap(vars(population_group),
...
### Import data file
DSOSOdt <- import(here("Datasets", "DSOSO_2021to4May2024.csv"))
...
**(Period: 2021/01/01 - 2024/05/03)**<br>

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.width=5, fig.align='center'}
### ***********************************************************
### ********** GRAPH 9 - TOTAL #s vs TIME line graph **********
### ***********************************************************

### Use "group by" and "sum" functions to sum up observations
### Total Shelter Population TIMELINE
df3NAtotals <- df3.NA %>% group_by(OCCUPANCY_DATE) %>%
...
plot.ResSum <- ggplot(df3NAtotals) +
  aes(x = OCCUPANCY_DATE, y = sum_Users,
      text = paste('Date: ', as.Date(OCCUPANCY_DATE),
                   '<br># Shelters: ', n_observ,
                   '<br># Shelter Occupants: ', sum_Users)) +
...
### Create Interactive Plot with "ggplotly" package
plotResSumly <- ggplotly(plot.ResSum, tooltip = "text")
...
**Figure 9. Total Shelter Population within the Greater Toronto Area (GTA).** The above plot shows the total number of daily shelter users for the period spanning 2021-01-01 to 2024-05-03. The number of shelter occupants have increased by more than 63% during this time.
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, out.width='85%', fig.align='center'}
### *************************************************************
### ********** GRAPH 10 - TOTAL # vs TIME (CITY FACETS) **********
### *************************************************************
...
### Create Interactive Plot with "ggplotly" package
plotCTly <- ggplotly(plotCT, tooltip = "text")
...
**Figure 10. Shelter Population per GTA region.** The plot shows the total number of shelter users within different parts of the **GTA** (i.e. Toronto, North York, Etobicoke, Scarborough, Vaughan) for the past ~41 months (2021-01-01 to 2024-05-03). The data reveals that the vast majority of shelter users are currently being housed within Toronto and North York (see Bar graph in **Figure 10**).
...
**(Regional Numbers: 2024-05-03)**<br>

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=4, fig.align='center'}
### *********************************************************
### ********** GRAPH 11 - BAR GRAPH TOTAL # vs CITY **********
### *********************************************************

### Use "group by" and "sum" functions to sum up observations
### Total Population per "CITY"
df240503 <- subset(df3.NA, OCCUPANCY_DATE == "2024-05-03")
CityTots <- df240503 %>%
  group_by(OCCUPANCY_DATE, CITY) %>%
  summarise(n_observ = n(), sum_Users = sum(SERVICE_USER_COUNT))
...
### Create Interactive Plot with "ggplotly" package
plotCityTotsly <- ggplotly(plotCityTots, tooltip = "text")
...
**Figure 11. Current Shelter Populations per GTA Region.** This bar graph shows the current (2024-05-03) number of shelter occupants within each of the designated **GTA** regions. Toronto (68.6%) and North York (18.2%) shelter the majority of homeless people, while Scarborough (8.7%), Etobicoke (3.9%), and Vaughan (0.6%) collectively house less than 14% of the homeless population.
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, out.width='85%', fig.align='center'}
### ***************************************************************
### ********** GRAPH 12 - TOTAL # vs TIME (SECTOR FACETS) **********
### ***************************************************************
...
### Create Interactive Plot with "ggplotly" package
plotSectorly <- ggplotly(plotSector, tooltip = "text")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, out.width='80%', fig.align='center'}
### ****************************************************************
### ******* GRAPH 13 - TOTAL # vs TIME (PROGRAM MODEL FACETS) *******
### ****************************************************************
...
### Create Interactive Plot with "ggplotly" package
plotPMtotalsly <- ggplotly(plotPMtotals, tooltip = "text")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=10, out.width='85%', fig.align='center'}
### ***************************************************************
### ******* GRAPH 14 - TOTAL # vs TIME (SERVICE TYPE FACETS) *******
### ***************************************************************
...
df3NA.3May2024 <- subset(df3NAservtype, OCCUPANCY_DATE == "2024-05-03")
...
### Create Interactive Plot with "ggplotly" package
plotSTtotalsly <- ggplotly(plotSTtotals, tooltip = "text")
...
**Figure 14. Homeless Overnight Service Types within the GTA.** Homeless individuals within the **GTA** can access different types of overnight services, including: **(i) Shelters**, **(ii) 24-Hour Respites**, **(iii) Motels/Hotels**, **(iv) Interim Housing**, **(v) Warming Centres**, **(vi) 24-Hour Women’s Drop-ins**, and **(vii) Isolation/Recovery Sites**. There are a few other **Service Types** offered, but they are ill-defined and accessed by very few people. As of May 3<sup>rd</sup> 2024 the vast majority of **Service Types** are emergency type **Shelter** (4852, or 48.7%) and **Motel/Hotel** (4522, or 45.4%) services. Unfortunately, services that help homeless people transition to safer and more affordable permanent housing are largely non-existent within the Toronto Shelter system.

<br>
<br>

<div align=center>
**Figure 15: GTA Shelter Population (DSOSO)**<br>
**(Program Area Numbers)**<br>

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, out.width='85%', fig.align='center'}
### ***************************************************************
### ******* GRAPH 15 - TOTAL # vs TIME (PROGRAM AREA FACETS) *******
### ***************************************************************
...
### Create Interactive Plot with "ggplotly" package
plotdf3NAPAly <- ggplotly(plotdf3NAPA, tooltip = "text")
...
### *****************************************************************
### ** GRAPH 16 - TOTAL # for PID16011 (2035 Kennedy Rd) 2024-03-29 **
### *****************************************************************
...
plot3 <- ggplot(data = df3NAPID16011,     # data set
  mapping = aes(                          # map aesthetics
    ...
    text = paste('Date: ', as.Date(OCCUPANCY_DATE),
                 ...
                 '<br>City: ', CITY,
                 '<br># Shelter Occupants: ', SERVICE_USER_COUNT))) +
  geom_point(aes(group=1), color = "darkblue", size = 2, alpha = 0.4)+
...
### Create Interactive Plot with "ggplotly" package
plot3bly <- ggplotly(plot3b, tooltip = "text")
...
<div align=center>
**Figure 17: Homes First - Mixed Adult COVID-19 Program, Delta Hotel Shelter**<br>
**(DSOSO: # Unavailable Rooms)**<br>

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, out.width='90%', fig.align='center'}
### *******************************************************************
### ** GRAPH 17 - TOTAL #s for PID16011 (2035 Kennedy Rd) 2024-03-29 **
### *******************************************************************
plot16011 <- df3NAPID16011 %>%
  filter(OCCUPANCY_DATE >= "2024-01-04" & OCCUPANCY_DATE <= "2024-05-03") %>%
  ggplot() +
  aes(x = OCCUPANCY_DATE, y = Rooms_Unavailable, colour = Room_Capacity,
      text = paste('Date: ', as.Date(OCCUPANCY_DATE),
                   '<br>Capacity: ', Room_Capacity,
                   '<br># Unavailable Rooms: ', Rooms_Unavailable)) +
...
### Create Interactive Plot with "ggplotly" package
plot2035Kenly <- ggplotly(plot16011, tooltip = "text")
